...
class MSAGrafter(object):
    ...
    def root_independent_tree(self, og_part, fn_tree_new, n_seqs_123many, name_orig, name_temp, iq_name_adjust):
        """
        ...
        Args:
            ...
            name_orig - original name given to gene or None
            name_temp - temporary name given to gene to avoid clashes or None
                        The name will be changed back from name_temp to name_orig
    ...
    def add_gene(self, og_part, infn, name_orig = None, name_temp = None, method = "iqtree"):
        ...
        genes = self.get_gene_names(fn_msa_new)
        if n_seqs_orig_123many >= 3:
            ...
            if not q_subtree:
                ...
                if name_orig is not None and name_temp is not None:
                    genes = [g.replace(name_temp, name_orig) for g in genes]
        ...
        if not q_subtree:
            ...
            fn_final_tree, warn_string_root = self.root_independent_tree(og_part, fn_tree_new, n_seqs_orig_123many, name_orig, name_temp, method == "iqtree")
            if warn_string != "":
                warn_string = "\n" + warn_string
            else:
            ...
            nwk_sub, t_sup = self.root_subtree(og_part, n_taxa_123many, genes, name_orig, name_temp, fn_tree_new)
            fn_final_tree = self.reconstruct_super_tree(infn, og_part, t_sup, nwk_sub)
        return fn_final_tree, warn_string


    def root_subtree(self, og_part, n_taxa_3many, genes, name_orig, name_temp, fn_tree_new):
    ...
    def reconstruct_super_tree(self, infn, og_part, t_sup, nwk_sub):
        ...
        Args:
            infn - original input fATA filename
